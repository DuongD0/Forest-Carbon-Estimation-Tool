...
class CRUDRole(CRUDBase[Role, RoleCreate, RoleUpdate]):
    ...
    def create(self, db: Session, *, obj_in: RoleCreate) -> Role:
        db_obj = Role(role_name=obj_in.role_name, description=obj_in.description)
        if obj_in.permission_ids:
            permissions = db.query(Permission).filter(Permission.permission_id.in_(obj_in.permission_ids)).all()
...
class CRUDUser(CRUDBase[User, UserCreate, UserUpdate]):
    ...
    def is_superuser(self, user: User) -> bool:
        return user.role and user.role.role_name.lower() in ["admin", "superuser"]
